...
error_t mpu6050_init(I2C_HandleTypeDef* i2cx) {
    ...
    HAL_I2C_Mem_Write(i2cx, MPU6050_ADDR, MPU6050_PWR_MGMT_1_REG, 1, &buf, 1, HAL_MAX_DELAY);

    // Set gyro config
    buf = (((1 << 2) - 1) & gyro_range) << 3;
    HAL_I2C_Mem_Write(i2cx, MPU6050_ADDR, MPU6050_GYRO_CONFIG_REG, 1, &buf, 1, HAL_MAX_DELAY);

    // Set accel config
    buf = (((1 << 2) - 1) & accel_range) << 3;
    HAL_I2C_Mem_Write(i2cx, MPU6050_ADDR, MPU6050_ACCEL_CONFIG_REG, 1, &buf, 1, HAL_MAX_DELAY);
...
void mpu6050_read(I2C_HandleTypeDef *i2cx, mpu6050_data_t *data) {
    ...
    HAL_I2C_Mem_Read(i2cx, MPU6050_ADDR, MPU6050_ACCEL_XOUT_H_REG, 1, buf, 14, HAL_MAX_DELAY);
